...
function castNumber(value, type) {
    ...
    if (specialNumber) {
        return new bignumber_js_1.BigNumber(specialNumber.result);
    }
...
function multiplyValue(value, times) {
    if (!times) {
        var stringifiedNumber = bigNumberToString(new bignumber_js_1.BigNumber(value));
        return floorStringifiedNumber(stringifiedNumber);
    }
    var bigNumProduct = new bignumber_js_1.BigNumber(value).times(new bignumber_js_1.BigNumber(times));
